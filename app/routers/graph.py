...
def _build_svg(
    ...
    """Combo chart — strict SVG layer order:
       grids → bars → fill_area → line_temp → dots → hit+tooltips → x_axis
    """
    ...
    # ── 5. CSS (inside SVG — HTMX safe) ──────────────────────────────────
    css = (
        "<style>"
        ".tip-col .tt{display:none;pointer-events:none;}"
        ".tip-col:hover .tt{display:block;}"
        ".tip-col:hover .hit-bg{fill:rgba(255,255,255,0.05);}"
        "</style>"
    )

    # ── 6. Grid lines ─────────────────────────────────────────────────────
    grids = ""
    for pct, lbl in [(100, "100%"), (50, "50%"), (0, "0%")]:
        yg = yp(float(pct))
        grids += (
    ...
    # ── 7. Pre-compute layout ─────────────────────────────────────────────
    slot_w     = chart_w / n if n > 1 else chart_w
    bar_w      = max(2.0, slot_w * 0.82)
    coords_t   = [(xc(i), yp(norm_t[i])) for i in range(n)]
    max_norm_t = max(norm_t)

    # ── LAYER A: Smoke bars (painted first — bottom layer) ────────────────
    bars = ""
    for i in range(n):
        bh = norm_s[i] / 100.0 * chart_h
    ...
    # ── LAYER B: Temperature fill area + Bezier line (above bars) ─────────
    ...
    # ── LAYER C: Temperature dots (above line) ────────────────────────────
    dots = ""
    for i, (cx_, cy_) in enumerate(coords_t):
        r = 4.5 if norm_t[i] == max_norm_t else 3.0
        dots += (
            f'<circle cx="{cx_:.1f}" cy="{cy_:.1f}" r="{r}" '
            f'fill="{COL_TEMP}" stroke="#1e1e20" stroke-width="1.5"/>'
        )

    # ── LAYER D: Hit zones + tooltip cards (top layer, interactive) ────────
    hit_cols = ""
    for i in range(n):
        cx_   = coords_t[i][0]
        pct_s = norm_s[i]
        ...
        tooltip = (
            ...
            f'<rect x="2" y="2" width="{TT_W}" height="{TT_H}" rx="8" fill="rgba(0,0,0,0.45)"/>'
            f'<rect width="{TT_W}" height="{TT_H}" rx="8" fill="#16181d" stroke="rgba(255,255,255,0.13)" stroke-width="1"/>'
            f'<text x="12" y="22" fill="#ffffff" font-size="13" font-weight="600" font-family="Outfit,sans-serif">{times[i]}</text>'
            f'<line x1="12" y1="30" x2="{TT_W - 12}" y2="30" stroke="rgba(255,255,255,0.10)" stroke-width="1"/>'
            f'<rect x="12" y="38" width="10" height="10" rx="2" fill="{COL_TEMP}"/>'
            f'<text x="28" y="48" fill="#e0e0e0" font-size="11.5" font-family="Outfit,sans-serif">Nhi\u1ec7t \u0111\u1ed9: {tt_temp}</text>'
            f'<rect x="12" y="56" width="10" height="10" rx="2" fill="{COL_SMOKE}"/>'
            f'<text x="28" y="66" fill="#e0e0e0" font-size="11.5" font-family="Outfit,sans-serif">Kh\u00f3i: {tt_smoke}</text>'
            f'</g>'
        )
    ...
    # ── Final assembly — STRICT LAYER ORDER ───────────────────────────────
    #   css → grids → bars(A) → fill+line(B) → dots(C) → hit+tooltips(D) → x_axis
    return (
        f'<svg viewBox="0 0 {_W} {_H}" xmlns="http://www.w3.org/2000/svg" '
        f'style="width:100%;height:auto;display:block;overflow:visible;">'
        f"{css}{grids}{bars}{fill_area}{line_temp}{dots}{hit_cols}{x_axis}"
        f"</svg>"
    )
