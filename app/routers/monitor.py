...
async def get_dynamic_threshold() -> int:
    """Tính toán ngưỡng động với Type Safety."""
...
async def monitor_system(data: MonitorPayload):
    ...
    if new_status == "critical":
        ...
        if inc_res.rows:
            # Gán vào biến tạm để kiểm tra kiểu dữ liệu an toàn
            inc_id_raw = inc_res.rows[0][0]
            old_peak_raw = inc_res.rows[0][1]

            inc_id = int(inc_id_raw) if isinstance(inc_id_raw, int) else 0
            old_peak = (
                float(old_peak_raw) if isinstance(old_peak_raw, (int, float)) else 0.0
            )
...
async def get_status_html():
    ...
    # Gán vào các biến tạm để tránh việc đánh giá kiểu dữ liệu bị lỗi
    raw_status = r[0]
    raw_temp = r[1]
    raw_smoke = r[2]
    raw_thresh = r[3]
    ...
    # Kiểm tra kiểu rõ ràng để thu hẹp kiểu (Type Narrowing)
    ...
    # Trả về các mẩu HTML mảnh (Ngắt dòng để tránh lỗi Ruff E501)
    temp_frag = (
